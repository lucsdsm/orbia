...
export default function ItemEdit() {
  ...
  const { id } = useParams();
  const { itens, updateItem } = useContext(ItensContext);
  const { cartoes } = useContext(CartoesContext);

  const [loading, setLoading] = useState(false);
  const [formData, setFormData] = useState(null);
  const [emojiPersonalizado, setEmojiPersonalizado] = useState('');
  ...
  useEffect(() => {
    ...
    if (item) {
      const emojiAtual = item.emoji || item.categoria || '💰';
      const isEmojiPadrao = emojis.includes(emojiAtual);
      ...
      setFormData({
        ...
        mesPrimeiraParcela:
          item.mesPrimeiraParcela ?? item.mes_primeira_parcela ?? new Date().getMonth(),
      });
      
      if (!isEmojiPadrao) {
        setEmojiPersonalizado(emojiAtual);
      }
    } else {
      ...
      <form className="itemform" onSubmit={handleSubmit}>
        ...
        <div className="form-group">
          ...
          <div className="emoji-grid">
            {emojis.map((emoji) => (
              <button
                key={emoji}
                type="button"
                className={`emoji-btn ${formData.emoji === emoji && !emojiPersonalizado ? 'selected' : ''}`}
                onClick={() => {
                  handleChange('emoji', emoji);
                  setEmojiPersonalizado('');
                }}
                disabled={loading}
              >
                {emoji}
          ...
          <input
            ...
            onChange={(e) => {
              setEmojiPersonalizado(e.target.value);
              if (e.target.value) {
